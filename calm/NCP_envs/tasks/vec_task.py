...
class VecTaskPython(VecTask):
    ...
    def step(self, actions):
        if isinstance(actions, list) or isinstance(actions, tuple):
            actions_tensor = tuple([torch.clamp(action, -self.clip_actions, self.clip_actions) for action in actions])
        else:
            actions_tensor = torch.clamp(actions, -self.clip_actions, self.clip_actions)
        return self.task.step(actions_tensor)

    def reset(self, env_ids=None):
        return self.task.reset(env_ids)
